...
export const ErrorState: Story = {
	...
	play: async ({ canvasElement }) => {
		const canvas = within(canvasElement);
		// getErrorMessage returns the default fallback for plain Error
		// objects (it only surfaces API-error payloads and raw strings).
		await waitFor(() => {
			expect(
				canvas.getByText(/unable to load debug panel data/i),
			).toBeInTheDocument();
		});
	},
};
