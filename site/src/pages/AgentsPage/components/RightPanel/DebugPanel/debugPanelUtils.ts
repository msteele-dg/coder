...
const toCodeContent = (value: unknown): string | undefined => {
	...
	// Preserve primitive values (numbers, booleans) as their
	// string representation so debug payloads are not silently
	// dropped.
...
const isToolCallPartType = (partType: string): boolean => {
	return partType === "tool-call" || partType === "tool_call";
};
